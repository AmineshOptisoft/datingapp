...
export default function CompanionDetailPage() {
  ...
  useEffect(() => {
    if (!legacyId) return;
    const profileId = `companion-${legacyId}`;
    fetch(`/api/characters/${profileId}/gifts`)
      .then(res => res.json())
      .then(data => { if (data.success && data.gifts) setGifts(data.gifts); })
      .catch(console.error);
  }, [legacyId]);
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-600 dark:text-zinc-400">The AI companion you're looking for doesn't exist.</p>
        </div>
      </div>
    );
  }
