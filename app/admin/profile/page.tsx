...
import AdminCreateSceneDialog from "./AdminCreateSceneDialog";
...
export default function ProfilePage() {
  ...
  const [selectedCharacterForScenes, setSelectedCharacterForScenes] = useState<any>(null);
  ...
  const fetchCharacters = async () => {
    ...
    try {
      ...
      const userId = pid || userData._id || userData.id || userData.userId;
  ...
  const fetchGifts = async () => {
    try {
      ...
      const response = await fetch(`/api/users/gifts-received?userId=${userId}`);
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    // Fallback to parsed user ID if no pid is provided
    const userId = pid || parsedUser._id || parsedUser.id || parsedUser.userId;
    
    if (pid) {
      // If we are viewing a specific user, just fetch their profile info
      fetchCurrentUser(pid, token);
    } else {
      // Viewing our own admin profile
      setUser(parsedUser);
      if (userId) {
    ...
    <main className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white pt-20 pb-10 px-4 flex flex-col items-center transition-colors">
      ...
      <div className="flex flex-col items-center w-full max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          {/* {pid && (
            <p className="text-orange-500 font-medium text-sm">Query PID: {pid}</p>
          )} */}
        ...
        <div className="flex items-center space-x-6 text-sm text-zinc-500 font-medium">
          ...
          <div className="hover:text-zinc-800 dark:hover:text-zinc-300 cursor-pointer transition-colors flex items-center gap-1">
            ...
            <span>{user?.totalInteractions || 0} Interactions</span>
          </div>
        </div>
        ...
        <div className="w-full py-2 text-center">
          ...
          {activeTab === 'scenes' && (
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col mx-12 items-center gap-2">
                <p className="text-zinc-900 dark:text-zinc-100 font-medium">Create a Scene (Admin)</p>
                <Dialog open={isCreateSceneDialogOpen} onOpenChange={setIsCreateSceneDialogOpen}>
                  ...
                  <DialogContent className="max-w-md max-h-[90vh] h-auto p-0 bg-transparent border-0" showCloseButton={false}>
                    <AdminCreateSceneDialog
                      userId={pid || user?._id || user?.id || user?.userId}
                      onSuccess={() => {
          ...
          {activeTab === 'gifts' && (
            <div>
              {loadingGifts ? (
                <div className="text-zinc-500 animate-pulse text-center py-8">Loading gifts...</div>
              ) : gifts.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {gifts.map((gift: any) => (
                    <div
                      key={gift._id}
                      className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 hover:shadow-lg hover:border-zinc-300 dark:hover:border-zinc-700 transition-all flex flex-col items-center gap-3"
                    >
                      {/* Gift Image */}
                      {gift.giftImage ? (
                        <img
                      ...
                      <div className="text-center">
                        <p className="font-semibold text-sm text-zinc-900 dark:text-white">{gift.giftName}</p>
                        <p className="text-xs text-yellow-600 dark:text-yellow-400 font-medium flex items-center justify-center gap-1">
                          <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#F59E0B"/><circle cx="12" cy="12" r="7" fill="#FBBF24"/><text x="12" y="16" textAnchor="middle" fontSize="12" fontWeight="bold" fill="#92400E">$</text></svg>
                          {gift.price} coins
                        </p>
                      </div>

                      {/* Sent to which character */}
                      {gift.character && (
                        <div className="flex items-center gap-2 w-full">
                          {gift.character.image ? (
                            <img src={gift.character.image} alt={gift.character.name} className="w-6 h-6 rounded-full object-cover flex-shrink-0" />
                          ) : (
                            <div className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center flex-shrink-0">
                              <User className="w-3 h-3 text-zinc-500" />
                            </div>
                          )}
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">To: {gift.character.name}</p>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 w-full">
                        {gift.sender?.avatar ? (
                          <img src={gift.sender.avatar} alt={gift.sender.name} className="w-5 h-5 rounded-full object-cover flex-shrink-0" />
                        ) : (
                          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center flex-shrink-0">
                            <span className="text-[8px] font-bold text-white">{gift.sender?.name?.[0] || '?'}</span>
                          </div>
                        )}
                        <p className="text-xs text-zinc-400 dark:text-zinc-500 truncate">From: {gift.sender?.name || 'Unknown'}</p>
                      </div>

                      {/* Date */}
                      <p className="text-[10px] text-zinc-400 dark:text-zinc-600">
                        {new Date(gift.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-16 text-zinc-400 dark:text-zinc-600">
                  ...
                  <p className="font-medium">No gifts received yet</p>
                  <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-1">Gifts sent to your characters will appear here</p>
                </div>
              )}
            </div>
          )}

          {activeTab !== 'characters' && activeTab !== 'personas' && activeTab !== 'scenes' && activeTab !== 'gifts' && (
            ...
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">All scenes featuring this character</p>
                </div>
              </div>
              <button 
                onClick={() => setIsCharacterScenesOpen(false)}
                  ...
                  {characterScenes.map((scene) => (
                    <div
                      ...
                      onClick={() => {
                        setViewingScene(scene);
                        setIsSceneViewOpen(true);
                        setIsCharacterScenesOpen(false); // Close this modal to show the global scene viewer
                      }}
                    >
                      {scene.mediaType === "image" ? (
                        <img
