...
export default function GirlCard({
  ...
  // Determine the route based on ID range
  // IDs 1-199 are girls, 200-299 are boys, 300+ are LGBTQ companions
  const getProfileRoute = (girlId: number) => {
    if (girlId >= 300) return `/companion/${girlId}`;
    if (girlId >= 200) return `/boy/${girlId}`;
    return `/girl/${girlId}`;
  };

  return (
    <Link
      href={getProfileRoute(id)}
